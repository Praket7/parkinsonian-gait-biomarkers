...
def gee_raw_effect(table, feature, *, minimum_participants=20):
    """Estimate native-unit change per one-point MDS-UPDRS gait increment."""
    frame, usable = _ready(table, feature, minimum_participants=minimum_participants)
    row = {"feature": feature, "raw_effect_per_gait_point": np.nan,
           "raw_ci_low": np.nan, "raw_ci_high": np.nan, "raw_p_value": np.nan,
           "raw_effect_status": NOT_ESTIMABLE}
    if not usable:
        return row
    try:
        fit = _fit(frame, f"{feature} ~ mds_updrs_gait_item + age + height_m + C(sex) + C(task) + C(site)")
        interval = fit.conf_int().loc["mds_updrs_gait_item"]
        row.update(raw_effect_per_gait_point=float(fit.params["mds_updrs_gait_item"]),
                   raw_ci_low=float(interval.iloc[0]), raw_ci_high=float(interval.iloc[1]),
                   raw_p_value=float(fit.pvalues["mds_updrs_gait_item"]), raw_effect_status="OK")
    except (ValueError, np.linalg.LinAlgError, KeyError):
        row["raw_effect_status"] = "FIT_FAILED"
    return row
...
def association_table(table, features, config):
    ...
    raw = pd.DataFrame([gee_raw_effect(table, feature, minimum_participants=minimum) for feature in features]).set_index("feature")
