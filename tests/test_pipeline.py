...
class PipelineTests(unittest.TestCase):
    ...
    def test_authorized_root_routes_to_v3_analysis(self):
        with tempfile.TemporaryDirectory() as tmp, patch("src.final_analysis.run_authorized_analysis", return_value={"status": "analysis_complete"}) as authorized:
            root = Path(tmp)
            cfg = root / "analysis.yaml"
            cfg.write_text("seed: 1\nprimary_tasks: [SP]\ncontext_tasks: [HP]\ndata:\n  output_dir: results\n")
