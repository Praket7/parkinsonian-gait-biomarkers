...
from src.v4_2.repeated_group_cv import favorable_fraction,run
def data(source):
 x=load(source);x['context_adjusted_gait_deviation_v1']=score(x,load_frozen_model('results/v4_1/frozen/v4_1_control_model.json'));x['severity']=pd.to_numeric(x.mds_updrs_gait_item,errors='coerce');return x[x.clinical_cohort.eq('pd')].dropna(subset=['severity','age','height_m','sex','task','site','gait_speed','context_adjusted_gait_deviation_v1']).copy()
def main():
 ...
 for level,g in result.groupby('level'):
  summary.append({'level':level,**{f'{col}_median':g[col].median() for col in g if col.startswith('delta_')},**{f'{col}_favorable_fraction':favorable_fraction(g,col.removeprefix('delta_')) for col in g if col.startswith('delta_')}})
